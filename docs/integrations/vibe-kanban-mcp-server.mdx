...
The `repositories` parameter is an array of repository objects, not an array of repository ID strings:
...
- `repo_id`: The repository ID (UUID) from `get_context().workspace_repos[]`
- `branch`: The branch or ref the new workspace should fork from for this repository

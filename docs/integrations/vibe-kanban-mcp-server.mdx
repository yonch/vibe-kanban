...
<Warning>
The HTTP transport exposes tools that mutate Vibe Kanban state. Always set `--http-token` (or `VIBE_KANBAN_MCP_HTTP_TOKEN`) when binding to anything other than `127.0.0.1`, or place the server behind a reverse proxy that enforces authentication.
</Warning>
...
| `--http-host` | `127.0.0.1` | Address to bind. Use `0.0.0.0` to accept remote connections. |
...
Point a remote client at `http://<host>:<port>/mcp`. For example, in Claude Desktop:

```json
{
  "mcpServers": {
    "vibe_kanban": {
      "url": "http://your-host.example.com:4096/mcp",
      "headers": {

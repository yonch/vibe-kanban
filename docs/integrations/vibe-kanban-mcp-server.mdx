...
<Warning>
The HTTP transport exposes tools that mutate Vibe Kanban state. Bearer tokens travel in the request, so any non-loopback deployment **must** terminate TLS in front of the server (a reverse proxy with a valid certificate, an SSH/VPN tunnel, or an authenticated tunnel like Cloudflare Access). Sending bearer credentials over plain `http://` to a remote host leaks the token to anyone on the network path. For loopback-only use (`127.0.0.1`), plain HTTP is fine.
</Warning>
...
Each flag has a matching environment variable: `VIBE_KANBAN_MCP_TRANSPORT`, `VIBE_KANBAN_MCP_HTTP_HOST`, `VIBE_KANBAN_MCP_HTTP_PORT`, `VIBE_KANBAN_MCP_HTTP_TOKEN`.
